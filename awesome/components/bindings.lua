...
bindings.mouse = gears.table.join(
  awful.button({}, 3, function() glob.menu:toggle() end),
...
bindings.keys = gears.table.join(
  ...
  awful.key({modkey}, "w", function() glob.menu:show() end, {description = "show main menu", group = "awesome"} ),
  ...
  awful.key({modkey}, "Return", function() awful.spawn(glob.terminal) end, {description = "open a terminal", group = "launcher"} ),
  ...
  awful.key({modkey}, "p", function() awful.spawn(glob.launcher, false) end, {description = "open launcher", group = "launcher"} ),
  awful.key({}, "Print", function() awful.spawn(glob.screenshot, false) end, {description = "Screenshot", group = "launcher"} )
)

bindings.clientkeys = gears.table.join(
  awful.key({modkey}, "F11", function(c) c.fullscreen = not c.fullscreen c:raise() end, {description = "toggle fullscreen", group = "client"} ),
...
for i = 1, 9 do
  bindings.keys = gears.table.join(
    bindings.keys,
    awful.key({modkey}, "#" .. i + 9, function() local screen = awful.screen.focused() local tag = screen.tags[i] if tag then tag:view_only() end end, {description = "view tag #" .. i, group = "tag"} ), -- Toggle tag display.
    awful.key({modkey, "Control"}, "#" .. i + 9, function() local screen = awful.screen.focused() local tag = screen.tags[i] if tag then awful.tag.viewtoggle(tag) end end, {description = "toggle tag #" .. i, group = "tag"} ), -- Move client to tag.
    awful.key({modkey, "Shift"}, "#" .. i + 9, function() if client.focus then local tag = client.focus.screen.tags[i] if tag then client.focus:move_to_tag(tag) end end end, {description = "move focused client to tag #" .. i, group = "tag"} ), -- Toggle tag on focused client.
